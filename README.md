...
AdCMD v4.3 **拒绝**在以下系统运行：

| 拒绝的系统 | 原因 | 替代方案 |
|-----------|------|----------|
| Windows 95 | 内核太旧 | 使用 [AdCMD for MS-DOS](../dos/) |
| Windows 98 | 内核太旧 | 使用 [AdCMD for MS-DOS](../dos/) |
| Windows ME | 内核太旧 | 使用 [AdCMD for MS-DOS](../dos/) |
| Windows NT 4.0 | 内核太旧 | 使用 [AdCMD for MS-DOS](../dos/) |
| Windows NT 3.x | 内核太旧 | 使用 [AdCMD for MS-DOS](../dos/) |
...
### MSVC
```cmd
cl AdCMD_v4_3.cpp /FeAdCMD.exe /link user32.lib gdi32.lib shell32.lib wininet.lib urlmon.lib gdiplus.lib advapi32.lib version.lib /SUBSYSTEM:WINDOWS
```

### MinGW
```bash
g++ AdCMD_v4_3.cpp -o AdCMD.exe -luser32 -lgdi32 -lshell32 -lwininet -lurlmon -lgdiplus -ladvapi32 -lversion -mwindows -std=c++17 -static -lpthread -Wl,--subsystem,windows
```
